...
const CELL_WIDTH: usize = 200;
const CELL_HEIGHT: usize = CELL_WIDTH * 9 / 16;
const CELL_SIZE: f32 = 6.0;

#[derive(Resource)]
struct Grid([[bool; CELL_WIDTH]; CELL_HEIGHT]);
...
fn setup(mut commands: Commands, mut grid: ResMut<Grid>) {
    ...
    grid.0 = [[false; CELL_WIDTH]; CELL_HEIGHT];
...
fn compute_next_generation(mut grid: ResMut<Grid>) {
    ...
    // copy grid
    let mut new_grid = [[false; CELL_WIDTH]; CELL_HEIGHT];
    for row in 0..grid.0.len() {
        for col in 0..grid.0[0].len() {
...
fn main() {
    App::new()
        ...
        .insert_resource(Grid {
            0: [[false; CELL_WIDTH]; CELL_HEIGHT],
        })
